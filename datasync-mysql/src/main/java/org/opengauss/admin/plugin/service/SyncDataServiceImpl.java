...
public class SyncDataServiceImpl {
    ...
    public boolean checkSyncSuccess(SyncConfigDto configDto){
        ...
        try {
            String url = String.format("jdbc:opengauss://%s:%s/%s?user=%s&password=%s",
                    configDto.getOgHost(), configDto.getOgPort(), configDto.getOgDatabase(),
                    configDto.getOgUser(), configDto.getOgPass());

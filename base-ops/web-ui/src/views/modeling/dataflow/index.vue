        ...
        <div class="tool mb-s">
          ...
          <div class="filter">
            <div class="filter-item">
              <a-input-search :max-length="140"  v-model="filter.search" :loading="filter.loading" allowClear @search="isFilter" @press-enter="isFilter" @clear="isFilter"
                :placeholder="$t('modeling.dataflow.index.5m77w0y5b3k0')" search-button />
            </div>
          </div>
        </div>
        <div class="table">
            ...
            >
              ...
              <template #operation="{ record }">
                <a-button size="mini" type="text" @click="toDetail(record)">
                  ...
                  <template #default>{{$t('modeling.dataflow.index.5m77w0y5caw0')}}</template>
                </a-button>
                <a-button size="mini" type="text" @click="openDialog('cu', 'update', record)">
  ...
  getList(sendData).then((res: KeyValue) => {
    ...
    if (Number(res.code) === 200) {
      ...
      res.rows.forEach((item: KeyValue) => {
        if (item.type === 1) item.typeText = t('modeling.dataflow.index.5m77w0y5rzk0')
        else if (item.type === 2) item.typeText = t('modeling.dataflow.index.5m77w0y5s2s0')
        else if (item.type === 3) item.typeText = t('modeling.dataflow.index.5m77w0y5s6o0')
        else if (item.type === 4) item.typeText = t('modeling.dataflow.index.5m77w0y5ghc0')
        else item.typeText = ''
      })
      list.data = res.rows
      console.log(res)

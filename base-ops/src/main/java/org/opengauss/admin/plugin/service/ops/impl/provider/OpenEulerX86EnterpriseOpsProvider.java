...
public class OpenEulerX86EnterpriseOpsProvider extends AbstractOpsProvider {
    ...
    public void upgrade(UpgradeContext upgradeContext) {
        ...
        try {
            rootSession = jschUtil.getSession(upgradeContext.getHostPublicIp(),
                    ...
                    "root", encryptionUtils.decrypt(upgradeContext.getRootPassword()))
                    .orElseThrow(
    ...
    private void upgradePreinstall(Session rootSession, UpgradeContext upgradeContext) {
        ...
        try {
            JschResult jschResult = jschUtil.executeCommand(userGroupCommand,
        ...
        try {
            ...
            JschResult jschResult = jschUtil.executeCommand(command, rootSession, upgradeContext.getRetSession());
            if (0 != jschResult.getExitCode()) {
                log.error("chown failed, exit code: {}, error message: {}",
                        jschResult.getExitCode(), jschResult.getResult());
    ...
    public void uninstall(UnInstallContext unInstallContext) {
        ...
        HostInfoHolder hostInfoHolder = unInstallContext.getHostInfoHolders()
                .stream()
                .filter(hostInfoHolder1 -> hostInfoHolder1.getHostEntity()
                        .getHostId().equalsIgnoreCase(opsClusterNodeEntity.getHostId()))
                .findFirst()
    ...
    private void rmSsh(List<HostInfoHolder> hostInfoHolders,
        ...
        for (HostInfoHolder hostInfoHolder : hostInfoHolders) {
            ...
            final Session session = jschUtil.getSession(currentHost.getPublicIp(),
                    currentHost.getPort(), rootUser.getUsername(),
                    encryptionUtils.decrypt(rootUser.getPassword()))
                    .orElseThrow(() -> new OpsException("The root user failed to establish a connection"));
            try {
    ...
    private void doRestartCluster(OpsClusterContext opsClusterContext) {
        ...
        try {
            JschResult jschResult = jschUtil.executeCommand(command,
                    opsClusterContext.getOpsClusterEntity().getEnvPath(), ommUserSession, retSession);
            if (0 != jschResult.getExitCode()) {
    ...
    private void doStartCluster(OpsClusterContext opsClusterContext) {
        ...
        try {
            JschResult jschResult = jschUtil.executeCommand(command,
                    opsClusterContext.getOpsClusterEntity()
                            .getEnvPath(), ommUserSession, retSession);
            if (0 != jschResult.getExitCode()) {
                throw new OpsException("startup error，exit code " + jschResult.getExitCode());
            }
        } catch (Exception e) {

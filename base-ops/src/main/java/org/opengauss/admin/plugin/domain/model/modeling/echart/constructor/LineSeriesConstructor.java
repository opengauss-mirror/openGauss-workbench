...
public class LineSeriesConstructor extends BaseSeriesConstructor {
    ...
    public LineSeriesConstructor(Dimension xDimension, List<Indicator> indicators, List<Dimension> dimensions, List<Map<String, Object>> queryData) {
    ...
    public void getDimensionValue() {
        ...
        queryData.forEach(item->{
            dimensions.forEach(dim->{
                ...
                if( !dim.getCustomFlag() && dimCategory != null && !Objects.equals(dimField, xDimension.getField())) {
        ...
        queryData.forEach(item->{
            ...
            if (xIndexList.size() > 0) {
                ...
                dimensions.forEach(dim->{
                    ...
                    categoryGroup.add(categoryDimensionValueMapper.get(dim.getField()+categoryValue));
                });

                //Concat all found category names as key
                String groupKey = String.join("", categoryGroup);

                if (seriesData.containsKey(groupKey))
                {

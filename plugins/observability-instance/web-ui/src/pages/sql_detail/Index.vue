...
import StatisticalInformation from './statistical_information/Index.vue';
import IndexSuggestionData from './index_suggestions/Index.vue';
import ObjectInformation from './object_information/Index.vue';
import SystemSource from './system_source/Index.vue';
import ImplementationPlanThis from './implementation_plan/Index.vue';
import SqlDiagnose from './sql_diagnose/Index.vue'
...
const { data: statisticalInfoRes, run: requestStatisticalInfo, loading: statisticalInfoLoading } = useRequest((sqlId: string | string[], dbid: string | string[]) =>
    ogRequest.get(`/observability/v1/topsql/detail?id=${dbid}&sqlId=${sqlId}`), { manual: true }
)
...
onMounted(() => {
    ...
    if (typeof sqlId === 'string' && typeof dbid === 'string') {
        ...
        monitorStore.tab = 1;
        monitorStore.instanceId = urlParam.dbid as string || "";
    }
...
watch(statisticalInfoRes, (res) => {
    if (res != null) {
        ...
        monitorStore.fixedRangeTime = [res.start_time, res.finish_time]
        ...
        <div class="sql-detail-tab">
            <el-tabs v-model="tab">
                ...
                <el-tab-pane :label="$t('sql.implementationPlan')" name="implementationPlan">
                    <ImplementationPlanThis v-if="tab === 'implementationPlan'" :sqlId="urlParam.sqlId" :dbid="urlParam.dbid" />
                </el-tab-pane>
                ...
                <el-tab-pane :label="$t('sql.objectInformation')" name="objectInformation">
                    <ObjectInformation v-if="tab === 'objectInformation'" :sqlId="urlParam.sqlId" :dbid="urlParam.dbid" />
                </el-tab-pane>
                <el-tab-pane :label="$t('sql.indexSuggestions')" name="indexSuggestions">
                    <IndexSuggestionData v-if="tab === 'indexSuggestions'" :sqlId="urlParam.sqlId" :dbid="urlParam.dbid" :sqlText="data.sqlText" />
                </el-tab-pane>
                <el-tab-pane :label="$t('sql.sqlDiagnose')" name="diagnose">
                    <SqlDiagnose v-if="tab === 'diagnose'" :sqlId="urlParam.sqlId" :dbid="urlParam.dbid" :dbName="dbName" :sqlText="data.sqlText"/>
                </el-tab-pane>
            </el-tabs>
        </div>
    </div>    

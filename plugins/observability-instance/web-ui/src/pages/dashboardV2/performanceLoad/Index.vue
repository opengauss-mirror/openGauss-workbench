<template>
  ...
  <div class="small-charts">
    <div style="display: flex; flex-direction: row; justify-content: space-between">
      <div class="container" :class="[cpuBackGroud]">
        ...
        <div
          class="value"
          v-if="timeTypeSelected != timeTypeSelection.CUSTOM && metricsData.cpu && metricsData.cpu.length > 0"
        >
          {{ cpuValue }}%
        </div>
      ...
      <div class="container" :class="[memoryBackGroud]">
        ...
        <div
          class="value"
          v-if="timeTypeSelected != timeTypeSelection.CUSTOM && metricsData.memory && metricsData.memory.length > 0"
        >
          {{ memoryValue }}%
        </div>
      ...
      <div class="container" :class="[networkBackGroud]">
        ...
        <div
          ...
          v-if="timeTypeSelected != timeTypeSelection.CUSTOM && metricsData.network && metricsData.network.length > 0"
        >
      ...
      <div class="container" :class="[ioBackGroud]">
        ...
        <div
          class="value"
          v-if="timeTypeSelected != timeTypeSelection.CUSTOM && metricsData.io && metricsData.io.length > 0"
        >
          {{ ioValue }}%
        </div>
      ...
      <div class="container" :class="[swapBackGroud]">
        ...
        <div
          class="value"
          v-if="timeTypeSelected != timeTypeSelection.CUSTOM && metricsData.swap && metricsData.swap.length > 0"
        >
          {{ swapValue }}%
        </div>
        <div class="detail" @click="goto(tabKeys.ResourceMonitorMemory)">
  ...
  <div style="position: relative">
    ...
    <el-tabs v-model="tab" class="tab2">
      ...
      <el-tab-pane :label="$t('session.blockSessions.tabTitle')" :name="1">
        ...
        >
          <el-table-column :label="$t('session.blockSessions.sessionID')" width="200">
            <template #default="scope">
  ...
  -->
          ...
          <el-table-column
            ...
            show-overflow-tooltip
          />
          <el-table-column prop="tid" :label="$t('session.waitEventTab.tid')" width="130" show-overflow-tooltip />
          <el-table-column :label="$t('session.waitEventTab.sessionid')" width="140" show-overflow-tooltip>
            <template #default="scope">
              <el-link type="primary" class="top-sql-table-id" @click="gotoSessionDetail(scope.row.sessionid)">
                {{ scope.row.sessionid }}
              </el-link>
...
import { getIndexMetrics, getTOPSQLNow, TopSQLNow, TransTable, BlockTable, WaitEvent } from '@/api/observability'
import { getWDRSnapshot, WDRSnapshots } from '@/api/wdr'
...
const getBackground = (data: LineData[]) => {
  if (timeTypeSelected.value === timeTypeSelection.value.CUSTOM) return 'cold'
...
const getLineColor = (data: LineData[]) => {
  if (timeTypeSelected.value === timeTypeSelection.value.CUSTOM) return '#83CBFF'
  if (data.length <= 0) return 'none'
...
watch(
  ...
  () => {
    ...
    topSQLData.value.forEach((element) => {
      if (element.duration[0] === '.') element.duration = '0' + element.duration
    })

...
// const tabHeaderW = computed(() => i18n.global.locale.value === 'en' ? 'calc(100% - 723px)' : 'calc(100% - 678px)')

const autoRefreshFn = () => {
    autoRefresh.value = !autoRefresh.value
}

const { data: opsClusterData } = useRequest(() =>
    ogRequest.get("/observability/v1/topsql/cluster"), { manual: false }
)

const { data: connectStatusData, run: runConnectStatus, loading: connectStatusLoadding } = useRequest((nodeId: String) =>
    ogRequest.get(`/observability/v1/topsql/connect/${nodeId}`), { manual: true }
)
        ...
        <el-tabs v-model="tab">
            <div class="tab-wrapper-container">
                ...
                <div class="tab-wrapper-filter">
                    ...
                    <el-date-picker
                        ref="datePickerRef"
                        :disabled="filters[tab].rangeTime !== -1"
                        type="datetimerange"
                        v-model="filters[tab].time"
                        :start-placeholder="$t('app.startDate')"
                        :end-placeholder="$t('app.endDate')"
                        :range-separator="$t('app.to')"
...
<style scoped lang="scss">
    ...
    .tab-wrapper {
        ...
        &-filter {
            ...
            padding: 0 10px;
            height: 40px;
            > div:not(:last-of-type), > span, > button {
                margin-right: 4px;
            }

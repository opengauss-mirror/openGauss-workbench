...
const { run: requestData, loading } = useRequest((query) => {
    if (tab.value !== 1) {
        return new Promise(() => {});
    }
...
useIntervalTime(() => { requestData(curFilter); }, computed(() => refreshTime.value * 1000))
    ...
    <div class="top-sql">
        ...
        <div class="top-sql-table" v-if="!errorInfo" v-loading="loading">
            <el-table :data="data.tableData" border>
                ...
                <el-table-column :label="$t('sql.startTime')" :formatter="(r: any) => moment(r.start_time).format('YYYY-MM-DD HH:mm:ss')" />
                <el-table-column :label="$t('sql.finishTime')" :formatter="(r: any) => moment(r.finish_time).format('YYYY-MM-DD HH:mm:ss')" />
                ...
                <el-table-column :label="$t('sql.excutionTime')" prop="execution_time" :width="i18n.global.locale.value === 'en' ? 150 : 105"></el-table-column>
            </el-table>
        </div>
        <my-message
            v-if="errorInfo"

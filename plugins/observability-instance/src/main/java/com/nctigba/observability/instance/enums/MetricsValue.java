...
public enum MetricsValue {
    ...
    NETWORK_TXPCK(Type.OS, "max(rate(agent_network_transmit_packets_total{host='ogbrench'}[5m])) by (device)",
            "{device}"),
    NETWORK_RX(Type.OS, "max(rate(agent_network_receive_bytes_total{host='ogbrench'}[5m])/ 1024) by (device)", "{device}"),
    NETWORK_TX(Type.OS, "max(rate(agent_network_transmit_bytes_total{host='ogbrench'}[5m])/ 1024) by (device)", "{device}"),

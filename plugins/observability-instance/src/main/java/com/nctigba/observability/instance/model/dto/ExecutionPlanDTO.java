 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-instance/src/main/java/com/nctigba/observability/instance/model/dto/ExecutionPlanDTO.java
 *
 *  -------------------------------------------------------------------------
 */
...
public class ExecutionPlanDTO {
    ...
    String joinType;
    // sub nodes
    List<ExecutionPlanDTO> children = new ArrayList<>();
     ...
     */
    public ExecutionPlanDTO(String executionPlan) {
     ...
     */
    private void processExecutionPlan(List<String> lines, int previousIndent, ExecutionPlanDTO plan,
            List<ExecutionPlanDTO> children) {
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (currentIndent > previousIndent) {
                lines.set(i, "");
                ExecutionPlanDTO subPlan = processExecutionPlanString(line);
     ...
     */
    private ExecutionPlanDTO processExecutionPlanString(String line) {
        if (line.contains(CommonConstants.HASH_COND)) {
    ...
    public int totalPlanRows() {
        int rows = planRows;
        for (ExecutionPlanDTO executionPlan : children) {

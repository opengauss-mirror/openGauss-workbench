...
/**
 * The monitoring implementation class of MonitoringHandler uses the self implemented bean name of monitoringType, such as prometheus<br>
 *
 * @author yangjie
 */
...
public class MonitoringHandlerFactory {
    ...
    @PostConstruct
    public void init() {
        handlerMap = handlerList.stream().collect(Collectors.toMap(MonitoringHandler::getMonitorType, Function.identity()));
    ...
    /**
     * Return the processing class of the monitoring according to the monitoring type. If it is not found, return the default processing class
     *
    ...
    public MonitoringHandler getInstance(String monitoringType) {
        return handlerMap.containsKey(monitoringType) ? handlerMap.get(monitoringType) : handlerMap.get(MonitoringType.DEFAULT.getMonitoringType());
    }
}

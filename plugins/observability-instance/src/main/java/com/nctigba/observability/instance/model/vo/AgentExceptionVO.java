/*
 *  Copyright (c) GBA-NCTI-ISDC. 2022-2024.
 *
 *  openGauss DataKit is licensed under Mulan PSL v2.
 *  You can use this software according to the terms and conditions of the Mulan PSL v2.
 *  You may obtain a copy of Mulan PSL v2 at:
 *
 *  http://license.coscl.org.cn/MulanPSL2
 *
 *  THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
 *  EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
 *  MERCHANTABILITY OR FITFOR A PARTICULAR PURPOSE.
 *  See the Mulan PSL v2 for more details.
 *  -------------------------------------------------------------------------
 *
 *  AgentExceptionVO.java
 *
 *  IDENTIFICATION
 *  plugins/observability-instance/
 *  src/main/java/com/nctigba/observability/instance/model/vo/AgentExceptionVO.java
 *
 *  -------------------------------------------------------------------------
 */

package com.nctigba.observability.instance.model.vo;

import lombok.Data;

/**
 * AgentExceptionVO
 *
 * @since 2024/2/19 16:57
 */
@Data
public class AgentExceptionVO {
    private String exceptionInfo;
    private boolean isUpStatus;

    /**
     * setAgentStatus
     *
     * @param isUpStatus boolean
     * @param exceptionInfo String
     * @return AgentExceptionVO
     */
    public AgentExceptionVO setAgentStatus(boolean isUpStatus, String exceptionInfo) {
        this.isUpStatus = isUpStatus;
        this.exceptionInfo = exceptionInfo;
        return this;
    }
}

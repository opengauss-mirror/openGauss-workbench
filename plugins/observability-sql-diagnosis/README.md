...
| 2025/08/09 | v1.5.0 | 支持存储历史慢SQL                              | 蒋宏博 | 王超   |
...
**步骤 3：** 在target目录下找到生成的observability-sql-diagnosis-{version}-repackage.jar，安装到一体化平台。
...
​	智能诊断插件主要有安装部署、慢SQL分析、慢SQL诊断和历史数据诊断等功能：

（1）安装部署主要包括代理的安装、卸载、启动、停止和状态监控、自动拉起功能。代理需要部署在对应的数据库服务器，且需要通过拥有Root权限的用户进行安装。

（2）慢SQL分析主要包括慢SQL趋势图、统计信息、明细信息功能，主要是对数据库中的历史慢SQL数据进行统计分析。慢SQL趋势图的数据依赖于后端存储的历史慢SQL数据。为防止datakit管理的数据库节点慢SQL信息丢失，插件定时对数据库节点的慢SQL信息进行采集，并存储在Sqlite中，同时为慢SQL设置合理的生命周期进行定时清理。慢SQL统计信息以及明细信息的数据依赖于数据库参数的设置，未设置时将提示对应的错误信息。
...
![image-20230313211135139](doc/sql_proxy_installed.PNG)
...
![image-20230313211135139](doc/stopping.PNG)
...
![image-20230313211135139](doc/starting.PNG)
...
​	2、通过Root用户安装的需要输入Root密码，点击【一键卸载】按钮，开始卸载操作，点击【取消】按钮，关闭卸载窗口。
...
![image-20230313210957759](doc/install_04.PNG)
...
智能诊断插件对datakit管理的数据库节点定时进行慢SQL采集并持久化，慢SQL采集功能依赖数据库开启慢SQL相关参数，若需采集某个数据库节点的慢SQL信息，请在对应节点设置如下参数：

enable_stmt_track:on 表示开启Full/Slow SQL捕获；

track_stmt_stat_level:xxx/L0 设置第2个参数为L0及以上级别；

log_min_duration_statement：设置合理的慢SQL阈值；

对于持久化后的慢SQL信息，智能诊断插件会根据用户配置的慢SQL生命周期进行定时清理。

#### 5.3.2查看慢SQL信息

​	1、智能诊断首页默认展示慢SQL分析的TAB页面，慢SQL生命周期、慢SQL采集频率为基础配置项。集群/实例、数据库和执行开始时间为查询条件，输入相关查询条件，点击【查询】按钮可以查看选择的实例下数据库的慢SQL趋势图、统计信息、明细信息，点击【重置】按钮会清除筛选条件。

**说明：**

| 配置项        | 必填 | 组件形式   | 配置说明                           |
| ------------- | ---- | ---------- | ---------------------------------- |
| 慢SQL生命周期 | 否   | 下拉框     | 慢SQL生命周期，默认值30天          |
| 慢SQL采集频率 | 否   | 下拉框     | 插件后台慢SQL采集频率，默认值5分钟 |
...
![image-20230313212433973](doc/sql_diagnosis_index.PNG)
...
![image-20230313212433973](doc/sql_diagnosis_create_task.PNG)
...
![image-20230313212433973](doc/sql_04.PNG)
...
![image-20230619114753111](doc/history_1.PNG)
...
![image-20230619114941211](doc/history_2.PNG)
...
![image-20230619115319451](doc/history_4.PNG)

![image-20230619115319451](doc/history_6.PNG)

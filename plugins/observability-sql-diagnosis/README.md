...
| 2023/06/14 | v1.1.0 | 新增历史数据诊断 | luomeng  | zengseliang |
...
**步骤 3：** 在target目录下找到生成的observability-sql-diagnosis-5.0.0-repackage.jar，安装到一体化平台。

![](doc/3.png)

### 4.2本地调试

​	目前插件依赖平台插件运行，暂不支持本地调试。
...
| 回调路径     | 否   | 文本输入框 | 平台的地址，代理回传数据时调用平台Api接口，有默认值 |
| 安装目录     | 否   | 文本输入框 | 代理安装在服务器上的绝对路径，有默认值              |
...
**说明：** 默认展示集群名称，点击【集群名称】可查看集群下已安装代理的实例
...
​	1、在【已安装代理】列表中，鼠标悬停在实例上会显示【卸载】按钮，点击【卸载】按钮，打开卸载窗口。

 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/collection/metric/DbAvgCpuItem.java
 *
 *  -------------------------------------------------------------------------
 */
...
public class DbAvgCpuItem extends PrometheusCollectionItem {
    ...
    public String getPrometheusParam(List<DiagnosisThresholdDO> thresholds) {
        if (CollectionUtils.isEmpty(thresholds)) {
            return MetricConstants.DB_AVG_CPU_USAGE_RATE;
        } else {
            StringBuilder sb = new StringBuilder(MetricConstants.DB_AVG_CPU_USAGE_RATE);
            for (DiagnosisThresholdDO threshold : thresholds) {
                if (ThresholdConstants.DB_CPU_USAGE_RATE.equals(threshold.getThreshold())) {
                    sb.append(">=");

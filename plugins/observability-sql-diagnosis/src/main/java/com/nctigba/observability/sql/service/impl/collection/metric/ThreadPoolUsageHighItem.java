 ...
 *  -------------------------------------------------------------------------
 *
 *  ThreadPoolUsageHighItem.java
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/collection/metric/ThreadPoolUsageHighItem.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class ThreadPoolUsageHighItem extends PrometheusCollectionItem {
    @Override
    public String getPrometheusParam(List<DiagnosisThresholdDO> thresholds) {
        if (CollectionUtils.isEmpty(thresholds)) {
            return MetricConstants.THREAD_POOL_USAGE_RATE;
        } else {
            StringBuilder sb = new StringBuilder(MetricConstants.THREAD_POOL_USAGE_RATE);
            for (DiagnosisThresholdDO threshold : thresholds) {
                if (ThresholdConstants.THREAD_POOL_USAGE_RATE.equals(threshold.getThreshold())) {
                    sb.append(">=");

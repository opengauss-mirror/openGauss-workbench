 ...
 *  -------------------------------------------------------------------------
 *
 *  BusinessConnCountItem.java
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/collection/metric/BusinessConnCountItem.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class BusinessConnCountItem extends PrometheusCollectionItem {
    @Override
    public String getPrometheusParam(List<DiagnosisThresholdDO> thresholds) {
        if (CollectionUtils.isEmpty(thresholds)) {
            return MetricConstants.BUSINESS_CONN_COUNT;
        } else {
            StringBuilder sb = new StringBuilder(MetricConstants.BUSINESS_CONN_COUNT);
            for (DiagnosisThresholdDO threshold : thresholds) {
                if (ThresholdConstants.CONNECTION_NUM.equals(threshold.getThreshold())) {
                    sb.append(">=");

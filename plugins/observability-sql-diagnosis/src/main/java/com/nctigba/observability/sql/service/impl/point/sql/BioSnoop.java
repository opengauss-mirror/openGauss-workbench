 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/sql/BioSnoop.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class BioSnoop implements DiagnosisPointService<Object> {
    @Autowired
    ...
    public AnalysisDTO analysis(DiagnosisTaskDO task, DataStoreService dataStoreService) {
        ...
        var center = new TaskResultDTO(
                task, TaskResultDTO.ResultState.NO_ADVICE, ResultTypeEnum.Biosnoop, FrameTypeEnum.Table,
                FrameVO.bearing.center);
        list.add(center);
        var table = TableFormatterUtils.format(file, "TIME(s)");
        ...
        if (!CollectionUtils.isEmpty(table.getData())) {
            table.getData().stream().anyMatch(row -> {
                var find = NumberUtils.toInt(row.get("PID"), 0) == task.getPid()
                        && NumberUtils.toFloat(row.get("LAT")) > 10;
                if (find) {
                    ...
                    top.setData(Map.of("title", LocaleStringUtils.format("Biosnoop.title"), "suggestions",
                            List.of(LocaleStringUtils.format("Biosnoop", row.get("FILENAME")))));
                    list.add(top);
                }
                return find;
            });

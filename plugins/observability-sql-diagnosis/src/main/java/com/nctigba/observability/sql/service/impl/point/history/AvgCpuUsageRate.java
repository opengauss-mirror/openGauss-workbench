 ...
 *  -------------------------------------------------------------------------
 *
 *  AvgCpuUsageRate.java
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/history/AvgCpuUsageRate.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class AvgCpuUsageRate implements DiagnosisPointService<List<PrometheusDataDTO>> {
    @Autowired
    ...
    public AnalysisDTO analysis(DiagnosisTaskDO task, DataStoreService dataStoreService) {
        ...
        List<?> list = (List<?>) dataStoreService.getData(avgCpuItem).getCollectionData();
    ...
    public List<PrometheusDataDTO> getShowData(int taskId) {
        ...
        for (PrometheusDataDTO dto : dataList) {
            if (dto.getChartName() != null && dto.getChartName().equals(MetricConstants.AVG_CPU_USAGE_RATE)) {
                dto.setUnit("%");
                dto.setChartName(LocaleStringUtils.format("history.AVG_CPU_USAGE_RATE.metric"));
                List<MetricDataDTO> datas = dto.getDatas();
                datas.forEach(f -> {
                    if (f.getName() != null && f.getName().equals(MetricConstants.AVG_CPU_USAGE_RATE)) {
                        f.setName(LocaleStringUtils.format("history.AVG_CPU_USAGE_RATE.metric"));
                    }
                });
            }
        }

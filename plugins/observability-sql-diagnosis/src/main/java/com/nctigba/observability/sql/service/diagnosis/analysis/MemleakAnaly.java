...
@Service
public class MemleakAnaly implements ResultAnalysis {
	@Autowired
	...
	public void analysis(GrabType grabType, Task task, MultipartFile file) {
		try {
			...
			TaskResult resultSvg = new TaskResult(task, ResultState.NoAdvice, ResultType.Memleak, FrameType.Flamefigure,
					bearing.top)
					.setData(Map.of("title", LocaleString.format("MemleakAnaly.TIP"), "id", resource.getId()));
			resultMapper.insert(resultSvg);
		} catch (IOException e) {
			throw new CustomException("Memleak err", e);
		}
	}
}
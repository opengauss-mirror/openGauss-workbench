 ...
 */
@Service
public class ColdFunction implements HisDiagnosisPointService<Object> {
    @Autowired
    ...
    private List<TaskResult> getResultData(HisDiagnosisTask task, TableData table) {
        ...
        if (table.getData().size() > 1) {
            ...
            var center = new TaskResult(
                    task, TaskResult.ResultState.NO_ADVICE, ResultType.ColdFunction, FrameType.Table,
            ...
            taskResult.setData(
                    Map.of(CommonConstants.TITLE, LocaleString.format("ColdFunction.title"), "suggestions",
                            LocaleString.format("ColdFunction.name")));
            list.add(center);
        }
        return list;
    }

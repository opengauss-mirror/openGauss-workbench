...
public class Biosnoop implements ResultAnalysis {
	private final DiagnosisTaskResultMapper resultMapper;
	private static final String TIP = "Biosnoop.TIP";

	@Override
	public void analysis(GrabType grabType, Task task, MultipartFile file) {
		var center = new TaskResult(task, ResultState.NoAdvice, ResultType.Biosnoop, FrameType.Table, bearing.center);
		var table = TableFormatter.format(file, "TIME(s)");
		...
		table.getData().stream().anyMatch(row -> {
			...
			if (find) {
				resultMapper.insert(new TaskResult(task, ResultState.Suggestions, ResultType.Xfsslower,
						FrameType.Suggestion, bearing.top)
						.setData(Map.of("title", LocaleString.format("Biosnoop.title"), "suggestions",

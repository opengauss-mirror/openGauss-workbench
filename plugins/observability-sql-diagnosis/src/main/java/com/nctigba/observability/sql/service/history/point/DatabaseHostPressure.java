 ...
 */
@Service
public class DatabaseHostPressure implements HisDiagnosisPointService<List<PrometheusDataDTO>> {
    @Autowired
    ...
    public AnalysisDTO analysis(HisDiagnosisTask task, DataStoreService dataStoreService) {
        ...
        if (count > 0) {
            dto.setIsHint(HisDiagnosisResult.ResultState.SUGGESTIONS);
        } else {
            dto.setIsHint(HisDiagnosisResult.ResultState.NO_ADVICE);
        }
        dto.setPointType(HisDiagnosisResult.PointType.CENTER);
        return dto;
    }

...
public interface HisDiagnosisResultMapper extends BaseMapper<HisDiagnosisResult> {
     ...
     * Batch insert data
     *
     * @param hisDiagnosisResult List of HisDiagnosisResult
     */
    @Insert("<script>"
            + "INSERT INTO his_diagnosis_result_info (id, cluster_id, node_id, task_id, point_name, point_type, "
            + "point_title, point_suggestion, point_data, is_hint, point_detail, point_state, is_deleted, create_time, "
            + "update_time) "
            + "VALUES "
            + "<foreach collection='list' item='item' separator=','>"
            + "(#{item.id}, #{item.clusterId}, #{item.nodeId}, #{item.taskId}, #{item.pointName}, #{item.pointType}, "
            + "#{item.pointTitle}, #{item.pointSuggestion}, "
            + "#{item.pointData, typeHandler = com.nctigba.common.mybatis.JacksonJsonWithClassTypeHandler}, "

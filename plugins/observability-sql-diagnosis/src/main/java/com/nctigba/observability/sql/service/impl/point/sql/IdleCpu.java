 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/sql/IdleCpu.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class IdleCpu implements DiagnosisPointService<Object> {
    ...
    private List<TaskResultDTO> getResultData(DiagnosisTaskDO task, MultipartFile file) {
        try {
            ...
            all.parse();
            idle.parse();
            TaskResultDTO idleResult = new TaskResultDTO(task, TaskResultDTO.ResultState.NO_ADVICE, ResultTypeEnum.idle,

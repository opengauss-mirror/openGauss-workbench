 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/sql/ObjectInfo.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class ObjectInfo implements DiagnosisPointService<AutoShowDataVO> {
    ...
    public AnalysisDTO analysis(DiagnosisTaskDO task, DataStoreService dataStoreService) {
        ...
        try (Connection conn = clusterManager.getConnectionByNodeId(task.getNodeId());
            ...
            while (rs.next()) {
                if (rs.getDouble(8) > THRESHOLD_VALUE) {

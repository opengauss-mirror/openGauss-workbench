 ...
 *  -------------------------------------------------------------------------
 *
 *  PartitionedSeqScan.java
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/sql/PartitionedSeqScan.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class PartitionedSeqScan implements DiagnosisPointService<Object> {
    @Autowired
    ...
    public AnalysisDTO analysis(DiagnosisTaskDO task, DataStoreService dataStoreService) {
        ...
        if (!sb.toString().contains("Partitioned Seq Scan on")) {
    ...
    private boolean isHasArithmetic(List<String> rsList, String column) {
        for (int i = 0; i < rsList.size(); i++) {
            ...
            boolean isHasOperator = explain.contains("Partitioned Seq Scan on");

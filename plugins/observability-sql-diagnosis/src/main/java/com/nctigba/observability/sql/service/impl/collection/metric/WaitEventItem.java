 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/collection/metric/WaitEventItem.java
 *
 *  -------------------------------------------------------------------------
 */
...
public class WaitEventItem extends PrometheusCollectionItem {
    ...
    public String getPrometheusParam(List<DiagnosisThresholdDO> thresholds) {
        if (CollectionUtils.isEmpty(thresholds)) {
            return MetricConstants.WAIT_EVENT;
        } else {
            StringBuilder sb = new StringBuilder(MetricConstants.WAIT_EVENT);
            for (DiagnosisThresholdDO threshold : thresholds) {
                if (ThresholdConstants.WAIT_EVENT_NUM.equals(threshold.getThreshold())) {
                    sb.append(">=");

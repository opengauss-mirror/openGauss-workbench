 ...
 */
@Service
public class AvgCpuUsageRate implements HisDiagnosisPointService<List<PrometheusDataDTO>> {
    @Autowired
    ...
    public List<PrometheusDataDTO> getShowData(int taskId) {
        ...
        for (PrometheusDataDTO dto : dataList) {
            if (dto.getChartName() != null && dto.getChartName().equals(MetricCommon.AVG_CPU_USAGE_RATE)) {
                dto.setUnit("%");
                dto.setChartName(LocaleString.format("history.AVG_CPU_USAGE_RATE.metric"));

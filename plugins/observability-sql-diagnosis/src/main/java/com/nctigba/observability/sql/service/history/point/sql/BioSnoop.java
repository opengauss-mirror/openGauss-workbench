 ...
 */
@Service
public class BioSnoop implements HisDiagnosisPointService<Object> {
    @Autowired
    ...
    public AnalysisDTO analysis(HisDiagnosisTask task, DataStoreService dataStoreService) {
        ...
        var center = new TaskResult(
                task, TaskResult.ResultState.NO_ADVICE, ResultType.Biosnoop, FrameType.Table, Frame.bearing.center);
        list.add(center);
        ...
        if (!CollectionUtils.isEmpty(table.getData())) {
            table.getData().stream().anyMatch(row -> {
                ...
                if (find) {
                    ...
                    top.setData(Map.of("title", LocaleString.format("Biosnoop.title"), "suggestions",
                            List.of(LocaleString.format("Biosnoop", row.get("FILENAME")))));
                    list.add(top);
                }
                return find;
            });

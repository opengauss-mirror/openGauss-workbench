 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/history/TopCpuTimeSql.java
 *
 *  -------------------------------------------------------------------------
 */
 ...
 */
@Service
public class TopCpuTimeSql implements DiagnosisPointService<Object> {
    @Autowired
    ...
    public List<?> getShowData(int taskId) {
        ...
        for (DatabaseVO dto : dataList) {
            if (dto.getSqlName() != null && dto.getSqlName().getString("__name__").equals(
                    SqlConstants.TOP_CPU_TIME_SQL)) {
                dto.setTableName(LocaleStringUtils.format("history.TOP_CPU_TIME_SQL.table"));
            }
        }
        return dataList;
    }

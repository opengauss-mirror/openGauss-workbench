...
import com.nctigba.observability.sql.service.history.collection.ebpf.CacheStatItem;
 ...
 */
@Service
public class CacheStat implements HisDiagnosisPointService<Object> {
    @Autowired
    ...
    public AnalysisDTO analysis(HisDiagnosisTask task, DataStoreService dataStoreService) {
        ...
        var center = new TaskResult(
                task, TaskResult.ResultState.NO_ADVICE, ResultType.cachestat, FrameType.Table, Frame.bearing.center);
        list.add(center);
        ...
        if (!CollectionUtils.isEmpty(table.getData())) {
            table.getData().stream().anyMatch(row -> {
                if (PercentUtil.parse(row.get("HITRATIO"), 1) < 0.9) {
                    var top = new TaskResult(task, TaskResult.ResultState.SUGGESTION, ResultType.cachestat,
                            FrameType.Suggestion, Frame.bearing.top)
                            .setData(Map.of("title", LocaleString.format("Cachestat.title"), "suggestions",
                                    List.of(LocaleString.format("Cachestat.TIP", task.getPid(), row.get("HITRATIO")))));
                    list.add(top);
                }
                return false;
            });

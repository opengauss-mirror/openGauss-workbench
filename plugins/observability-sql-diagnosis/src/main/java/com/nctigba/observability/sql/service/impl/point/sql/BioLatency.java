 ...
 *
 *  IDENTIFICATION
 *  plugins/observability-sql-diagnosis/src/main/java/com/nctigba/observability/sql/service/impl/point/sql/BioLatency.java
 *
 *  -------------------------------------------------------------------------
 */
...
import com.nctigba.observability.sql.service.impl.collection.ebpf.BioLatencyItem;
 ...
 */
@Service
public class BioLatency implements DiagnosisPointService<Object> {
    @Autowired
    ...
    public AnalysisDTO analysis(DiagnosisTaskDO task, DataStoreService dataStoreService) {
        ...
        var heatMap = HistogramToHeatmapUtils.format(file, LocaleStringUtils.format("Biolatency.unit"));
        ...
        heatMap.forEach(map -> {
            ...
            if (rate > 1.0f) {
                suggestions.add(LocaleStringUtils.format("Biolatency.tip", map.getName(), rate, 5));
            }
        });
        List<TaskResultDTO> list = new ArrayList<>();
        if (!suggestions.isEmpty()) {
            TaskResultDTO result = new TaskResultDTO(task, TaskResultDTO.ResultState.SUGGESTION,
                    ResultTypeEnum.Biolatency,
                    FrameTypeEnum.Suggestion,
                    FrameVO.bearing.top);
            result.setData(Map.of("title", LocaleStringUtils.format("Biolatency.title"), "suggestions", suggestions));
            list.add(result);
        }

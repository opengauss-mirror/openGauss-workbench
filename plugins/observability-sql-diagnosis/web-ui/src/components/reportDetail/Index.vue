<template>
    <div class="report-container">
        <div class="report-header">{{ getContent(data.currentReportScene.title, data.curTableName, props.reportId, 'title') }}</div>
        <div class="report-main">
            <div class="report-main-header">
                ...
                <div class="report-main-suggestion" v-if="!['ObjectInfoCheck', 'ExecPlan'].includes(props.reportId as string)">
                    ...
                    <span class="report-main-suggestion-text">{{ getContent(data.currentReportScene.advise, data.curTableName, props.reportId, 'advise') }}</span>
                </div>
                ...
                <div v-if="['ExecPlan'].includes(props.reportId as string)">
                    ...
                    <StatisticalInformation :large="large" v-if="taskInfo.debugQueryId" :sqlId="taskInfo.debugQueryId" :dbid="taskInfo.nodeId" />
                </div>
            </div>
            <div class="report-main-body" :style="`max-height: ${bodyHeight}px`">
                <div class="report-main-box" v-if="judgeIsShowMadal(['BaseInfo', 'ObjectStructure', 'PartitionInformation', 'IndexInformation'])">
                    <div class="report-main-box-header">{{ getMostCostTableTitle(data.curTableName, props.reportId) }}</div>
                    <BaseInfo :metaData="data.tableMetaData" :nodeType="props.reportId" :key="props.reportId" v-if="judgeIsShowMadal(['BaseInfo'])" />
                    <ObjectStructure :structureData="data.tableStructureData" v-if="judgeIsShowMadal(['ObjectStructure'])" />
                    <PartitionInformation :partitionInfoList="data.partitionInfoList" v-if="judgeIsShowMadal(['PartitionInformation'])" />
                ...
                <div class="report-main-box" v-if="judgeIsShowMadal(['RowsDiff', 'WorkMemDiff', 'ExplainAnalyze'])">
                    <RowsDiff :rowdDiffList="data.rowdDiffList" :changeCurChooseRow="changeCurChooseRow" v-if="judgeIsShowMadal(['RowsDiff'])" />
                    <WorkMemDiff :workmemAndPeakmemArr="data.workmemAndPeakmemArr" v-if="judgeIsShowMadal(['WorkMemDiff'])" />
                    <ExplainAnalyze :planTextArr="data.planTextArr" :curChooseRow="curChooseRow" v-if="judgeIsShowMadal(['ExplainAnalyze'])" />
                </div>
            </div>
        </div>
    </div>
...
import { allReportSceneList, reportSceneType, tableStructureType, tableIndexType, nativePlanType, partitionInfoType, rowsDiffType } from './common'
...
const getContent = (fullStr: string, tableName: string, nodeType: string, type: string) => {
    if (nodeType === 'PlanRecommendedToCreateIndex' && Array.isArray(data.indexAdvicesData)) {
        if (type === 'title') {
            return data.indexAdvicesData.length > 0 ? dealIndexAdviceSuggestion(data.indexAdvicesData) : t('report.noIndexSuggestions')
        }
        return data.indexAdvicesData.length > 0 ? `${dealIndexAdviceSuggestion(data.indexAdvicesData)}，${t('report.ImproveQueryEfficiency')}` : t('report.noIndexSuggestions')
    }
...
onMounted(() => {
    nextTick(() => {
        const domRect = document.querySelector('.og-card-body')?.getBoundingClientRect() as DOMRect
        bodyHeight.value = Math.floor(domRect.height) - 120
    })
    init()
})
...
const getMostCostTableTitle = (tableName: string, nodeType: string) => {
    if (nodeType === 'PlanRecommendedToCreateIndex' && Array.isArray(data.otherStructureAndIndexList) && data.otherStructureAndIndexList.length > 0) {
...
const dealPlanText = (query: string, rowsDiffClone: Array<rowsDiffType>) => {
    ...
    rowsDiffClone.forEach((item, index) => {
        if (!item.use && query.includes(item.stepName) && query.includes(item.estimateRows) && query.includes(item.actualRows)) {
            item.use = true
            tempQuery = tempQuery.replace(item.stepName, `<span id='row-${index}' style="color: #177DDC">${item.stepName}</span>`).replace(`rows=${item.estimateRows}`, `<span style="color: #177DDC">rows=${item.estimateRows}</span>`).replace(`rows=${item.actualRows}`, `<span style="color: #177DDC">rows=${item.actualRows}</span>`)
        }
    })
    return tempQuery
}
...
watch(diagnosisData, (res) => {
    ...
    const { peakMem = '', queryPlan = [], rowsDiff = [], partitionData = [], workMem = '', maxCostTableName = '', tableIndexData = {}, tableMetaData = {}, tableStructureData = {}, indexAdvicesData = [], otherStructureAndIndexList = [], jsonQueryPlan = {} } = executionPlan
    let tempPlanText = []

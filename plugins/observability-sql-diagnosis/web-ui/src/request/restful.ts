...
const isSuccessResponse = (res: any) => {
    if (res.status === 200) {
        if (res.data === undefined) {
            ...
            Object.keys(res.data).length === 2 &&
            typeof res.data.code !== 'undefined' &&
...
export class Request {
    ...
    async patch<T = any, D = any>(url: string, data?: D, config?: AxiosRequestConfig<D>) {
        ...
        try {
            return handleRESTfulData<T>(
                await axios.patch<ApiResponse<T>>(`${observabilityBaseURL}${url}`, data, config)
            )
        } catch (error) {
            const err = error as AxiosError<any>
            if (err.response && err.response.data && err.response.data.msg) {

<template>
    <div class="search-form">
        ...
        <div class="filter">
            ...
            <el-select v-model="formData.dbName" clearable style="width: 150px" :placeholder="$t('datasource.selectDatabaseType')">
                <el-option v-for="item in dbList" :key="item" :value="item" :label="item" />
            </el-select>
        </div>
        <div class="filter">
            <el-input v-model="formData.searchText" clearable style="width: 180px" :prefix-icon="Search" :placeholder="$t('datasource.searchPlaceholder')" />
        </div>
        <div class="filter">
    ...
    <div class="page-container">
        <div class="table-wrapper">
                    ...
                    <template #default="scope">
                        <span v-if="scope.row.name && scope.row.name.length > 18">
                            <el-popover width="300" trigger="hover" :content="scope.row.name" popper-class="sql-popover-tip">
                                <template #reference>
                                    <a class="table-wrapper-table-id" @click="gotoTaskDetail(scope.row.id)">{{ scope.row.name.substr(0, 18) + '...' }}</a>
                                </template>
                            </el-popover>
                        </span>
            ...
            <TrackAdd :type="1" v-if="addModel" :show="addModel" :dbName="formData.dbName" :clusterId="formData.cluster" :clusterList="clusterList" :dbList="dbList" @changeModal="changeModalCurrent" @conveyFlag="bandleCovey" />
        </div>
    </div>
</template>
...
const dbList = ref<Array<any>>(['postgres'])

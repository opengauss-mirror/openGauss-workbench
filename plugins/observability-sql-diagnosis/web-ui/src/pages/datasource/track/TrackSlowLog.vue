<template>
    <div class="search-form">
        ...
        <div class="filter">
            ...
            <el-select v-model="formData.dbName" :placeholder="$t('datasource.selectDatabaseType')" style="max-width: 150px" clearable>
        ...
        <el-button type="primary" class="search-button" @click="handleQuery" :icon="Search">{{ $t('app.query') }}</el-button>
        <el-button @click="handleReset" :title="$t('app.refresh')" :icon="Refresh">{{ $t('app.reset') }}</el-button>
    </div>
    <div class="slow-log">
        <div class="slow-log-table">
            <el-table size="small" :data="tableDatas" style="width: 100%" :default-sort="{ prop: 'date', order: 'descending' }">
            ...
            <el-pagination v-model:currentPage="page.currentPage" v-model:pageSize="page.pageSize" :total="page.total" :page-sizes="[10, 20, 30, 40]" class="pagination" layout="total,sizes,prev,pager,next" background small @size-change="handleSizeChange" @current-change="handleCurrentChange" />
        </div>
    </div>
    <TrackAdd :type="3" v-if="addModel" :show="addModel" :dbName="addTaskParam.dbName" :clusterId="[addTaskParam.clusterId, addTaskParam.nodeId]" :sqlText="addTaskParam.sqlText" @changeModal="changeModalCurrent" @conveyFlag="bandleCovey" />
</template>
...
const { data: res, run: requestData } = useRequest(
    () => {
        ...
        return ogRequest.get('/sqlDiagnosis/api/v1/slowSqls', { ...queryData.value }).then(function (res) {

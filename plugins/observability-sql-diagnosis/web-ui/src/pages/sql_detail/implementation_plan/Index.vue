...
import ogRequest from '../../../request/instance'
...
const { data: planDataRes, run: requesPlanDataRes, error, loading: planDataLoading } = useRequest((sqlId: string | string[], dbid: string | string[]) => ogRequest.get(`/observability/v1/topsql/plan?id=${dbid}&sqlId=${sqlId}`), { manual: true })
...
watch(error, () => {
    if (error.value != null) {
        errorInfo.value = error.value as unknown as string
    }
})
...
<template>
    <div class="implementation-plan" v-if="error == null">
        ...
        <div class="i-p-table" v-loading="planDataLoading">
            <el-table ref="singleTableRef" :data="data.planData" :style="{ width: '100%', marginBottom: '20px' }" row-key="id" :row-class-name="tableRowClassName" height="240" default-expand-all border>
                        ...
                        <div class="i-p-table-cost">
                            <div style="flex: 1; position: relative; height: 10px">
                                <my-progress
                                    ...
                                    :style="{ position: 'absolute', zIndex: row.singleCost / totalSingleCost <= row.totalCost / totalCost && row.nodeType !== 'Limit' ? 1 : 0 }"
                                    width="100%"
                                    height="10px"
                                ...
                                <my-progress
                                    :data="[
                                        { label: $t('report.singleStepOperationCost'), value: row.totalCost, color: '#0093FF', hide: row.nodeType === 'Limit' },
    ...
    <my-message v-if="error != null" :type="isInfoTip(errorInfo) ? 'info' : 'error'" :tip="$t(`sql.${errorInfo}`)" defaultTip="" :key="errorInfo" />
</template>

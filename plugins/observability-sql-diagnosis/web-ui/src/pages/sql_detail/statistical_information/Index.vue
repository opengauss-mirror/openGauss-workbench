...
import { consumingOption } from './common'
import { useRequest } from 'vue-request'
import instanceRequest from '../../../request/instance'

const { data: statisticalInfoRes, run: requestStatisticalInfo, loading: statisticalInfoLoading } = useRequest(() => instanceRequest.get(`/observability/v1/topsql/detail?id=${props.dbid}&sqlId=${props.sqlId}`), { manual: true })
...
onMounted(() => {
    requestStatisticalInfo()
})
</script>

<template>
    <div class="statistical-information" v-loading="statisticalInfoLoading">
        <div class="s-i-row">
                ...
                <my-card class="my-card" :title="$t('sql.consumingBreakdownTitle')" :bodyPadding="false">
                    <div class="s-i-consuming">
                        <div class="s-i-consuming-pie">
                            <my-pie :showLegend="false" :center="['50%', '50%']" :data="data.useTimeStatistical" :color="['#3DC94C', '#DA864C']" />
                        </div>
                        <div class="s-i-consuming-legend" v-if="large">
                            <div style="width: 43%" v-if="data.useTimeStatistical && data.useTimeStatistical.length > 0">
                                <div class="s-i-consuming-item large" style="width: 100%" v-for="item in data.useTimeStatistical" :key="item.value">
                            ...
                            <div style="width: 57%">
                                <div class="s-i-consuming-item large" style="width: 100%">
                                    ...
                                    <span class="s-i-consuming-item-value">{{ `${getValueForTime(requestData.statisticalInfo['db_time'])} ms` }}</span>
                                </div>
                                <div class="s-i-consuming-item large" style="width: 100%">
                                    <span class="s-i-consuming-item-label">{{ $t('sql.cpuTimeLabel') }}：</span>
                                    <span class="s-i-consuming-item-value">{{ `${getValueForTime(requestData.statisticalInfo['cpu_time'])} ms` }}</span>
                                </div>
                                <div class="s-i-consuming-item large" style="width: 100%">
                                    <span class="s-i-consuming-item-label">{{ $t('sql.waitTimeLabel') }}：</span>
                                    <span class="s-i-consuming-item-value">{{ `${getValueForTime(requestData.statisticalInfo['wait_time'])} ms` }}</span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="s-i-col-right">
                <my-card class="my-card" :title="$t('sql.consumingBreakdownTitle')" :bodyPadding="false">
                    <div class="s-i-consuming">
                        <div class="s-i-consuming-pie">
                            <my-pie :showLegend="false" :center="['50%', '50%']" :data="data.consumingPieData" :color="data.consumingPieColor" />
                        </div>
                        <div class="s-i-consuming-legend" v-if="large">
                            <div class="s-i-consuming-item large" v-for="item in consumingOption" :key="item.value">
                                ...
                                <span class="s-i-consuming-item-value">{{ `${getValue(item.value, requestData.statisticalInfo)} ms` }}</span>
                            </div>
                        </div>
                        <div class="s-i-consuming-legend" v-else>

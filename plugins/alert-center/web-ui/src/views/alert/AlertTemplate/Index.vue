...
const { data: templateRes, run: requestTemplateData } = useRequest(
    () => {
        ...
        return request.get("/alertCenter/api/v1/alertTemplate", params)
    },
    { manual: true }
);
const { data: ruleRes, run: requestRuleData } = useRequest(
    () => {
        ...
        return request.get(`/alertCenter/api/v1/alertTemplate/${currentId.value}/rule`, params)
    },
    { manual: true }
);
...
const delTemplate = (id: number) => {
    ElMessageBox.confirm(
        ...
        request.delete(`/alertCenter/api/v1/alertTemplate`, { id }).then((res: any) => {
            if (res && res.code === 200) {
                ElMessage({
                    type: 'success',

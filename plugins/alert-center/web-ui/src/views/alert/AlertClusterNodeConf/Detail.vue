        ...
        <el-dialog v-model="visible" :show-close="false" width="30%">
            ...
            <template #footer>
                <span>
                    ...
                    <el-button type="primary" @click="saveTemplateAndConfig">{{ t('app.confirm') }}</el-button>
                </span>
            </template>
        </el-dialog>
    </div>
...
const { data: res, run: requestData } = useRequest(
    (clusterNodeId) => {
        return request.get(`/alertCenter/api/v1/alertClusterNodeConf/clusterNode/${clusterNodeId}`)
    }
)

const { data: templateRes, run: requestTemplateData } = useRequest(
    () => {
        return request.get("/alertCenter/api/v1/alertTemplate/list")
    },
    { manual: true }
);
const { data: ruleRes, run: requestRuleData } = useRequest(
    () => {
        return request.get(`/alertCenter/api/v1/alertTemplate/${currentId.value}/rule/list`)
    },
    { manual: true }
);
watch(res, (res: any) => {
...
const confirm = () => {
    if (activeName.value === 'template') {
        ...
        request.post(`/alertCenter/api/v1/alertClusterNodeConf`, param).then((res: any) => {
            if (res && res.code === 200) {
                ElMessage({
...
const saveTemplateAndConfig = () => {
    ...
    request.post(`/alertCenter/api/v1/alertClusterNodeConf/alertTemplate`, param).then((res: any) => {
        if (res && res.code === 200) {
            ElMessage({
                message: t('app.saveSuccess'),
                type: 'success'

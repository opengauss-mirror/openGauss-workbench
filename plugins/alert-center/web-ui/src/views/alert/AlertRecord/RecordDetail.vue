...
const markAs = (val: any) => {
    if (formData.value.id) {
        if (val === 0) {
            request.post(`/alertCenter/api/v1/alertRecord/markAsUnread?ids=${formData.value.id}`).then((res: any) => {
                if (res && res.code === 200) {
        ...
        if (val === 1) {
            request.post(`/alertCenter/api/v1/alertRecord/markAsRead?ids=${formData.value.id}`).then((res: any) => {
                if (res && res.code === 200) {
                    requestData(formData.value.id)
                }
            })

<template>
    ...
    <el-menu class="vem-menu" :default-active="route.path" router :collapse="props.collapse" :collapse-transition="false"
        :style="{ width: collapse ? 'auto' : '200px' }">
        <div v-for="route in appData.curRoutes" :key="route.path">
            <el-sub-menu v-if="route.children && route.children.length > 0" :index="route.path">
                <template #title>
                    <svg-icon :name="route.meta?.icon!" style="margin-right: 6px" />

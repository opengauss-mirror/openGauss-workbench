    ...
    <el-dialog v-model="dialogVisible" :show-close="false">
      <el-row>
        <el-radio-group v-model="exportType">
          <el-radio :label="1">{{ $t('alertRecord.exportList') }}</el-radio>
          <el-radio :label="2">{{ $t('alertRecord.exportReport') }}</el-radio>
        </el-radio-group>
      </el-row>
      <template #footer>
...
const { data: pieRes, run: requestPisData } = useRequest(
  () => {
    ...
    return request.get("/api/v1/alertRecord/statistics", param)
  },
  { manual: true }
);
...
const { data: res, run: requestData } = useRequest(
  () => {
    ...
    return request.get("/api/v1/alertRecord", params)
  },
  { manual: true }
);
watch(res, (res: any) => {
...
const markAsUnread = () => {
  ...
  if (rows.length === 0) {
    ...
    request.post(`/api/v1/alertRecord/markAsUnread?ids=${ids}`).then((res: any) => {
      if (res && res.code === 200) {
...
const markAsRead = () => {
  ...
  if (rows.length === 0) {
    ...
    const ids = rows.map((item: any) => item.id).join(',')
    request.post(`/api/v1/alertRecord/markAsRead?ids=${ids}`).then((res: any) => {
      if (res && res.code === 200) {
        requestStatisticsData()
        requestPisData()
        requestData()
...
const showDetail = (id: number) => {
  if (process.env.mode === 'production') {
    window.$wujie?.props.methods.jump({
      name: `Static-pluginAlert-monitorVemAlertRecordDetail`,
      query: {
        id,
      },
    });
...
const exportTable = () => {
  ...
  if (exportType.value === 1) {
    request.get("/api/v1/alertRecord/export", params, { responseType: 'blob' }).then((res: any) => {
      if (res) {
        ...
        a.href = herf
        a.download = t('alertRecord.title') + ".xlsx"
    ...
    request.get("/api/v1/alertRecord/exportReport", params, { responseType: 'blob' }).then((res: any) => {
      if (res) {
        const blob = new Blob([res], {
          type: 'text/plain',
        })
        ...
        const herf = URL.createObjectURL(blob)
        a.href = herf
        a.download = t('alertRecord.title') + ".html"
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        window.URL.revokeObjectURL(herf)

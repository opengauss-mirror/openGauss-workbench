...
| 2023/09/19 | v2.0.0 | 基于v1.0.0进行修改 | 巫悦彬 | 曾色亮 |
...
- 展示告警规则列表，支持增删改查。
- 已内置规则：CPU使用率、内存使用率、磁盘使用率、IO、网络、数据库状态、数据库连接数、数据库阻塞会话数、慢SQL、表空间容量、数据库LOCK个数和锁时间的告警。
...
- 展示通知方式列表，支持增删改查。
- 支持邮箱、企业微信、钉钉、Webhook、SNMP等通知方式。
...
3、在target目录下找到生成的alert-monitor-5.1.0-repackage.jar，安装到一体化平台

...
public class SqlConstants {
    ...
    public static final String GET_SCHEMA_NAME_SQL = "SELECT oid, nspname FROM pg_namespace where nspname not in ('blockchain','snapshot','dbe_perf','pkg_service','cstore','pg_toast');";
    ...
    public static final String GET_DATA_Connection_NOT_P_SQL = "select id, type , name , driver , ip, port ,dataname , username, '' as userpassword ,webuser,edition from DATABASELINK WHERE";
    ...
    public static final String TABLE_SEQUENCE_SQL = "select t.tableuser, t.tableName, t.columnName, decode(seq.sequence_name, t.relname, seq.sequence_name, '[No_Privilege]') as sequenceName, decode(seq.sequence_name, t.relname, seq.sequence_schema, '[No_Privilege]') as sequenceuser, decode(seq.sequence_name, t.relname, seq.minimum_value, '[No_Privilege]') as minValue, decode(seq.sequence_name, t.relname, seq.maximum_value, '[No_Privilege]') as maxValue, decode(seq.sequence_name, t.relname, seq.increment, '[No_Privilege]') as increment" + LF +
            "  from (SELECT tu.rolname as tableuser, tb.relname as tableName, tc.attname as columnName, scl.relname, sch.nspname" + LF +
            "          FROM pg_class scl, pg_depend sdp, pg_attrdef sc, pg_attribute tc, pg_class tb, pg_roles tu, pg_namespace sch" + LF +
            ...
            "  LEFT OUTER join information_schema.sequences seq on (t.nspname = seq.sequence_schema AND t.relname = seq.sequence_name);";
    ...
    public static final String TABLE_PARTITIONG_ATTRIBUTE_SQL = "SELECT tbl.oid," + LF +
            ...
            "       tbl.relisshared as is_shared,split_part(split_part(array_to_string(tbl.reloptions, ','),',', 1),'=',2) orientation ," + LF +
    ...
    public static final String CONSTRAINT_UNIQUE_IMMEDIATE_SQL = " ALTER TABLE %s.%s ADD  CONSTRAINT %s UNIQUE (%s)  DEFERRABLE INITIALLY IMMEDIATE  ";
    public static final String CONSTRAINT_PRIMARY_SQL = " ALTER TABLE %s.%s ADD CONSTRAINT %s PRIMARY KEY (%s) ";
    public static final String CONSTRAINT_PRIMARY_IMMEDIATE_SQL = " ALTER TABLE %s.%s ADD CONSTRAINT %s PRIMARY KEY (%s)  DEFERRABLE INITIALLY IMMEDIATE  ";
    public static final String CONSTRAINT_CHECK_SQL = "   ALTER TABLE %s.%s ADD CONSTRAINT %s CHECK (%s)  ";
    public static final String CONSTRAINT_PARTIAL_CLUSTER_KEY_SQL = "   ALTER TABLE %s.%s  ADD CONSTRAINT %s  PARTIAL  CLUSTER KEY (%s)  ";
    public static final String CONSTRAINT_NO_CHECK_SQL = "   ALTER TABLE %s.%s ADD CONSTRAINT %s  %s  ";

    public static final String CONSTRAINT_FOREIGN_KEY_SQL = " ALTER TABLE %s.%s ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s.%s (%s) ";
    public static final String CONSTRAINT_TABLE_SQL = "select con.conname, string_agg(DISTINCT att.attname, ',' order by att.attname) as attname, con.contype, pg_get_constraintdef(con.oid) constraintdef," +
            " con.condeferrable, d.description,f.nspname AS ref_nspname , f.relname AS ref_tbname , string_agg(DISTINCT f.attname, ', ') AS ref_columns " +
            " from pg_class cla inner join pg_attribute att on cla.oid = att.attrelid and att.attnum > 0 inner join (select oid, condeferrable, " +
            " unnest(conkey) as conkey, conrelid, contype, conname,confrelid ,confkey from pg_constraint) con on att.attnum = con.conkey and con.conrelid " +
            " = cla.oid left JOIN   (SELECT pg_attribute.attnum,pg_attribute.attrelid,pg_attribute.attname,relname, nspname FROM pg_class INNER JOIN pg_namespace " +
            " ON pg_namespace.oid = pg_class.relnamespace INNER JOIN pg_attribute ON pg_class.oid = pg_attribute.attrelid)  f ON f.attrelid = con.confrelid " +
            " AND f.attnum = ANY ( con.confkey) inner join pg_namespace ns on cla.relnamespace = ns.oid and ns.nspname = '%s' LEFT JOIN pg_description d " +
            " ON con.oid = d.objoid  where cla.relname = '%s' group by con.conname,con.contype, pg_get_constraintdef(con.oid), con.condeferrable, ns.nspname " +
            " ,d.description,ref_nspname,ref_tbname";
    public static final String UNIQUE_CONSTRAINT_TABLE_SQL = "select  con.contype from pg_class cla inner join pg_attribute att on cla.oid = att.attrelid and att.attnum > 0" + LF +
            "inner join (select oid, condeferrable, unnest(conkey) as conkey, conrelid, contype, conname,confrelid ,confkey from pg_constraint) con " + LF +
            "on att.attnum = con.conkey and con.conrelid = cla.oid inner join pg_namespace ns on cla.relnamespace = ns.oid and ns.nspname = '%s' " + LF +
    ...
    public static final String INDEX_TABLE_SQL = "select pc.relname,pi.indisunique, am.amname, CASE WHEN pg_catalog.pg_get_expr(pi.indexprs, pi.indrelid, true) is null THEN string_agg(att.attname, ',' order by att.attname) ELSE null END attname, " +
            " pg_catalog.pg_get_expr(pi.indexprs, pi.indrelid, true) expression,  pg_description.description " +
            "from PG_INDEX pi inner join PG_CLASS pc on pi.indexrelid = pc.oid and pc.relkind = 'i' inner join PG_ATTRIBUTE att on pc.oid = att.attrelid " +
    ...
    public static final String SELECT_SEQUENCE_DDL_SQL = " SELECT sequence_schema,sequence_name,start_value,increment,minimum_value,maximum_value,cycle_option" + LF +
    ...
    public static final String DATABASE_ATTRIBUTE_SQL = "select ds.oid,ds.datname,pg_encoding_to_char(ds.encoding) as encoding,ds.datallowconn," + LF +
    ...
    public static final String SEQUENCE_SQL = "SELECT start_value, increment_by, max_value, min_value, is_called FROM %s;";

...
public class ConnectionUtils {
     ...
     * 获取数据库连接
     *
     * @param sysConfig 数据库连接配置
     * @return 数据库连接
     */
    public static Connection getConnection(SysConfig sysConfig) {
        CompletableFuture<Connection> future = CompletableFuture.supplyAsync(() -> {
            Connection connection = null;
            try {
                Class.forName(sysConfig.getDriver());
                connection = DriverManager.getConnection(sysConfig.getUrl(), sysConfig.getUserName(),

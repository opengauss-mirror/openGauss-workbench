...
import { OpenLookengInstallConfig, MinimalistInstallConfig, LiteInstallConfig, EnterpriseInstallConfig, InstallModeEnum, BenchMarkEnum, DeployTypeEnum, WsConnectTypeEnum } from '@/types/ops/install'
...
export const installStore = defineStore('install', {
  ...
  getters: {
    ...
    getInstallParam: (state): KeyValue => {
      ...
      if (state.installContext.installMode === InstallModeEnum.OFF_LINE) {
        param.installContext.minimalistInstallConfig = state.minimalistInstallConfig
      } else if (state.installContext.installMode === InstallModeEnum.ON_LINE) {

...
public class ZabbixServiceImpl {
    ...
    private static final String INSERT_HOST = "insert into hosts(hostid,host,status,available,name,description) values(%s,'%s',0,1,'%s','');";
    ...
    private static final String INSERT_ITEM = "insert into items(itemid,type,hostid,name,key_,delay,value_type,params,interfaceid,description,posts,headers,master_itemid,timeout,url) values(%s,%s,%s,'%s','%s','%s',%s,'',%s,'','','',%s,'%s','%s');";

    private static final String HOSTID_BY_HOST_NAME = "select hostid from hosts where host = '%s';";

    private static final String ITEMID_BY_MASTER_HOSTID = "select itemid from items where hostid = %S and name = 'Prometheus_node_metric';";
    ...
    private static final String UPDATE_ID_item_preproc = "update ids set nextid = %S  where field_name = 'item_preprocid';";
    ...
    public ZabbixMessge insertNecessary(String name, String ip, String port) {
        ...
        String itemSql = String.format(Locale.ROOT, INSERT_ITEM, itemid, 19, hostid, METRIC_NAME, METRIC_NAME, "10s", 4, interfaceid, null, "30m", url);
    ...
    private void insertOneTarget(ZabbixMessge zabbixMessge, List<ZabbbixMap> zabbbixMaps, String only, String hoatName) {
    ...
    private List<String> getAllSql(List<ZabbbixMap> total, ZabbixMessge zabbixMessge) {
        ...
        if (CollectionUtil.isNotEmpty(total)) {
            ...
            Integer father = zabbixMessge.getFather_item_id();
            Integer nextItem = wholeIds.getItemid() + 1;
            Integer nextProc = wholeIds.getItem_preprocid() + 1;
            for (ZabbbixMap zabbbixMap : total) {
                String sonItemSql = String.format(Locale.ROOT, INSERT_ITEM, nextItem, 18, hostId, zabbbixMap.getZabbixTargetName(), zabbbixMap.getZabbixKey(), "0", 0, null, father, "10s", "");
                ...
                String processSql = String.format(Locale.ROOT, INSERT_PROC, nextProc, nextItem, zabbbixMap.getZabbixTargetProcess());
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                for (Map.Entry<String, Object> entry : arry.entrySet()) {
                    if (entry.getValue().toString().matches(ISNUM) || entry.getValue().toString().matches(KEXUE)) {
                        ...
                        zabbixTargetName.append(entry.getKey()).append("_").append(zabbixSql.getOnly()).append("_").append(hostName);
                        zabbixProcess.append(zabbixTargetName).append(end).append("\n");
                        ZabbbixMap zabbbixMap = new ZabbbixMap(zabbixTargetName.toString(), zabbixKey.toString(), zabbixProcess.toString());

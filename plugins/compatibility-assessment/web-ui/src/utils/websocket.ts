...
export default class Socket<T, RT> extends Heart {
  ...
  create(): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      // change by yourself
      wsUrl = `ws://${process.env.VUE_APP_WS_BASE_URL}/ws/compatibility-assessment/${this.options.url}`
    } else {
      const wsPrefix = window.location.protocol.includes('https') ? 'wss' : 'ws'
      wsUrl = `${wsPrefix}://${window.location.host}/ws/compatibility-assessment/${this.options.url}`
    }

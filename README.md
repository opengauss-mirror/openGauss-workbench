...
本项目是基于Web的openGauss的可视化的一体化平台系统，目的是方便客户使用和管理openGauss可视化工具，可以为客户降低openGauss数据库安装使用门槛，做到安全中心管理，插件管理，以及其它功能包括一键化部署、卸载、组件化安装、多版本升级、日常运维和。
...
├── openGauss-visualtool  //一体化平台项目
├── openGauss-visualtool-plugin  //一体化平台项目插件开发脚手架
```
## 说明
1、插件需要安装在一体化平台上运行，因此需要先将一体化平台项目安装部署后，再将其他插件项目打包安装使用。

2、插件开发脚手架项目是为了方便开发者快速开发与一体化平台适配的插件，而搭建的插件开发脚手架，开发者可在此脚手架之上开发业务功能。该脚手架配置的各项依赖版本已经经过验证，和平台兼容性最好，因此建议不要修改依赖版本。
